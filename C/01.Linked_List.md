...
当链表中结点数为奇数`n`时，上述`while`循环结束时，快结点`fast`指向第`n+1`个元素，移动的次数为`(n+1)/2`，因此慢结点指向第`(n+1)/2`个结点。

当链表中结点数为偶数`n`时，`while`循环结束时，快结点`fast`指向第`n`个元素，移动的次数为`n/2`，因此慢结点指向第`n/2`个结点。
